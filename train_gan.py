...
parser.add_argument('--ganType', default='standard',help='dcgan or wgan')
parser.add_argument('--blockType', default='resnet',help='convnet resnet')
parser.add_argument('--ngpu', type=int,default=1,help='no. gpus to use')
parser.add_argument('--nImages', type=int,default=0,help='no. images to use')
...
parser.add_argument('--ganReg', default='none',help='fake, real, wgangp or none')
...
parser.add_argument('--gpAlpha',type=float, default=10, help='factor for gp loss term')
...
if opt.nImages:
    dataset.samples = dataset.samples[:opt.nImages]
    dataset.imgs = dataset.imgs[:opt.nImages]
...
# TODO: add parallelization to newtorks, specify ngpus in input parameter
print(netG)
print(netD)
...
output_folder = './results_lsgan_split/'
if os.path.isdir(output_folder) == False:
    os.mkdir(output_folder)

output_folder  = output_folder+f'./blockType{opt.blockType}_dName{opt.dName}_ganType{opt.ganType}_ganReg{opt.ganReg}_gpAlpha{opt.gpAlpha}_imageSize{opt.imageSize:04d}_lr{opt.lr:.1e}_batchSize{opt.batchSize:04d}/'
...
for epoch in range(opt.niter):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f GP(D)%.4f'
    ...
    bench_times.close()
    # do checkpointing
    torch.save(netG_test,output_folder + 'netG.pth')
